// Vendored from afedotov/gan-web-bluetooth v3.0.2 (MIT) src/gan-cube-protocol.ts
// 裁剪说明:
//   - 移除 Web Bluetooth 连接类 GanCubeClassicConnection / BluetoothDeviceWithMAC / GanCubeConnection
//     (连接生命周期由本项目 src/ble/transport/ 统一传输层负责)
//   - 移除 RxJS 依赖 (drivers 本身是纯函数, 返回事件数组)
//   - 保留: 事件类型定义 / GanProtocolMessageView 位视图 / Gen2 / Gen3 / Gen4 协议驱动
...
/** Command message */
export type GanCubeCommand = GanCubeReqHardwareCommand | GanCubeReqFaceletsCommand | GanCubeReqBatteryCommand | GanCubeReqResetCommand;
...
/** All possible event message types (本项目不使用 GYRO 事件, 已裁剪) */
export type GanCubeEventMessage = GanCubeMoveEvent | GanCubeFaceletsEvent | GanCubeBatteryEvent | GanCubeHardwareEvent;
/** Cube event / response to command */
export type GanCubeEvent = { timestamp: number } & GanCubeEventMessage;

/** Raw connection interface used by protocol drivers to send commands / disconnect */
 ...
 */
export class GanProtocolMessageView {
 ...
 *  - GAN356 i Carry 2
 */
export class GanGen3ProtocolDriver implements GanProtocolDriver {
 ...
 *  - GAN14 ui FreePlay
 */
export class GanGen4ProtocolDriver implements GanProtocolDriver {
        ...
        if (eventType == 0x01) { // MOVE
            ...
            switch (eventType) {
                case 0xFA: // Product Date
                    ...
                    this.hwInfo[eventType] = padNum(year, 4) + "-" + padNum(month, 2) + "-" + padNum(day, 2);
                    break;
